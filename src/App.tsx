...
import { X, MapPin, Phone, Mail, Leaf, Ruler, Award } from "lucide-react";
    ...
    <>
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-40 flex flex-col items-center justify-center gap-8" style={{ background: "#f5f0ea" }}>
          {["Projects", "Philosophy", "Gallery", "Contact"].map((link) => (
      ...
      <section ref={heroRef} style={{
        ...
        <div className="hero-text-block" style={{
          ...
          <div className="hero-heading-top flex items-end justify-between px-6 md:px-10 lg:px-16" style={{ marginBottom: "-0.04em" }}>
            ...
            <p className="hero-subtitle-desktop font-primary text-right" style={{ fontWeight: 700, fontSize: "clamp(10px, 0.95vw, 14px)", maxWidth: 300, opacity: 0.7, lineHeight: 1.6, marginBottom: "0.2em", letterSpacing: "0.02em", display: "none" }}>
              Passive House architecture with vision,<br />craft, and site-specific precision.
            </p>
          </div>
          <div style={{ overflow: "hidden" }}>
            <h1 className="font-primary hero-extraordinary uppercase px-6 md:px-10 lg:px-16" style={{ fontWeight: 800, color: "#000", letterSpacing: "-0.03em", margin: 0 }}>
              PERMANENCE
            </h1>
          </div>
          <p className="hero-subtitle-mobile font-primary px-6" style={{ fontWeight: 600, fontSize: "clamp(12px, 3vw, 15px)", opacity: 0.65, marginTop: "0.9em", display: "none" }}>
            Passive House architecture with vision,<br />craft, and site-specific precision.
          </p>
        </div>
      </section>
      ...
      <section ref={projectsRef} style={{ background: "#111", position: "relative", zIndex: 26 }} className="py-20 md:py-28 px-6 md:px-10 lg:px-16">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-16">
            <div>
              <p className="font-secondary text-sm mb-4" style={{ color: "rgba(255,255,255,0.4)", letterSpacing: "0.1em", textTransform: "uppercase" }}>Current Work</p>
            ...
            <p className="font-secondary text-sm max-w-sm" style={{ color: "rgba(255,255,255,0.5)", lineHeight: 1.7 }}>
              Three active projects in Austin, TX — each pursuing the highest standards of energy performance and architectural craft.
            </p>
          </div>
                ...
                <div className="p-6">
                  <div className="flex flex-wrap gap-2 mb-3">
                    {project.tags.map((tag) => (
                      <span key={tag} className="font-secondary text-[11px] px-2.5 py-1 rounded-full" style={{ background: "rgba(33,49,56,0.08)", color: BRAND_DARK }}>
                        {tag}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    <span className="font-secondary text-xs flex items-center gap-1" style={{ color: "rgba(33,49,56,0.6)" }}>
                      <MapPin size={12} /> {project.location}
                    </span>
                    <span className="font-secondary text-xs" style={{ color: "rgba(33,49,56,0.6)" }}>{project.size}</span>
                    <span className="font-secondary text-[10px] px-2 py-0.5 rounded-full" style={{ background: "#e8f5e9", color: "#2e7d32" }}>{project.status}</span>
                  </div>
                  <p className="font-secondary text-sm leading-relaxed" style={{ color: "rgba(33,49,56,0.7)" }}>{project.description}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
            <div className="flex items-start gap-4">
              <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: "rgba(33,49,56,0.08)" }}>
                <Leaf size={20} style={{ color: BRAND_DARK }} />
              </div>
              <div>
                <h4 className="font-primary text-lg font-bold" style={{ color: BRAND_DARK }}>Passive House</h4>
                <p className="font-secondary text-sm mt-1" style={{ color: "rgba(33,49,56,0.6)", lineHeight: 1.6 }}>PHIUS-certified design achieving extraordinary energy performance without sacrificing architectural ambition.</p>
              </div>
            </div>
            <div className="flex items-start gap-4">
              <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: "rgba(33,49,56,0.08)" }}>
                <Ruler size={20} style={{ color: BRAND_DARK }} />
              </div>
              <div>
                <h4 className="font-primary text-lg font-bold" style={{ color: BRAND_DARK }}>Site-Responsive</h4>
                <p className="font-secondary text-sm mt-1" style={{ color: "rgba(33,49,56,0.6)", lineHeight: 1.6 }}>Every project begins with the land. Solar path, prevailing winds, existing ecology — the site shapes the solution.</p>
              </div>
            </div>
            <div className="flex items-start gap-4">
              <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: "rgba(33,49,56,0.08)" }}>
              ...
              <div>
                <h4 className="font-primary text-lg font-bold" style={{ color: BRAND_DARK }}>Certified Excellence</h4>
                <p className="font-secondary text-sm mt-1" style={{ color: "rgba(33,49,56,0.6)", lineHeight: 1.6 }}>LEED AP BD+C, AIA, CPHC. Experience at KPF, Studio Libeskind, Marvel Architects, and SRA.</p>
              </div>
            </div>
          </div>

          <div style={{ height: 1, background: "linear-gradient(90deg, transparent 0%, rgba(33,49,56,0.15) 20%, rgba(33,49,56,0.15) 80%, transparent 100%)" }} className="mb-12" />

          <div className="flex flex-wrap items-start justify-between gap-12">
            <div>
              <h4 className="font-primary text-lg font-bold mb-1" style={{ color: BRAND_DARK }}>Sergey Belov</h4>
              <p className="font-secondary text-sm" style={{ color: "rgba(33,49,56,0.6)" }}>AIA, LEED AP BD+C, CPHC</p>
              <p className="font-secondary text-sm" style={{ color: "rgba(33,49,56,0.6)" }}>Principal + Founder</p>
              <p className="font-secondary text-xs mt-2" style={{ color: "rgba(33,49,56,0.45)" }}>Cornell University, B.Arch</p>
              <p className="font-secondary text-xs" style={{ color: "rgba(33,49,56,0.45)" }}>UT Austin Design Lead</p>
            </div>
            <div>
              <h4 className="font-primary text-lg font-bold mb-1" style={{ color: BRAND_DARK }}>Stephanie Belov</h4>
              <p className="font-secondary text-sm" style={{ color: "rgba(33,49,56,0.6)" }}>Co-Founder — Interiors</p>
              <p className="font-secondary text-xs mt-2" style={{ color: "rgba(33,49,56,0.45)" }}>MA Sustainable Interior Design, UT</p>
            </div>
            <div>
              <h4 className="font-primary text-lg font-bold mb-3" style={{ color: BRAND_DARK }}>Get in Touch</h4>
              <div className="space-y-2">
                <a href="mailto:info@saintelmoarch.com" className="font-secondary text-sm flex items-center gap-2 hover:opacity-70 transition-opacity" style={{ color: "rgba(33,49,56,0.7)" }}>
                  <Mail size={14} /> info@saintelmoarch.com
                </a>
                <a href="tel:+19175731747" className="font-secondary text-sm flex items-center gap-2 hover:opacity-70 transition-opacity" style={{ color: "rgba(33,49,56,0.7)" }}>
                  <Phone size={14} /> (917) 573-1747
                </a>
                <p className="font-secondary text-sm flex items-center gap-2" style={{ color: "rgba(33,49,56,0.5)" }}>
                  <MapPin size={14} /> Austin, TX 78745 + New York, NY
                </p>
              </div>
            </div>
          </div>
      ...
      <footer style={{ background: "#1a1a1a", position: "relative", zIndex: 26 }} className="py-12 px-6 md:px-10 lg:px-16">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="flex items-center gap-6">
            ...
            <span className="font-secondary text-xs" style={{ color: "rgba(255,255,255,0.3)" }}>Passive House + LEED + AIA + CPHC</span>
          </div>
          <div className="flex items-center gap-6">
            <a href="https://www.linkedin.com/in/sergeybelovarchitect" target="_blank" rel="noopener noreferrer" className="font-secondary text-sm hover:opacity-100 transition-opacity" style={{ color: "rgba(255,255,255,0.5)" }}>LinkedIn</a>
            <a href="https://saintelmoarch.com" target="_blank" rel="noopener noreferrer" className="font-secondary text-sm hover:opacity-100 transition-opacity" style={{ color: "rgba(255,255,255,0.5)" }}>Website</a>
            <a href="mailto:info@saintelmoarch.com" className="font-secondary text-sm hover:opacity-100 transition-opacity" style={{ color: "rgba(255,255,255,0.5)" }}>Contact</a>
          </div>
        </div>
      </footer>
    </>
